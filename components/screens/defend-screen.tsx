"use client";

import { DEFEND_DATA } from "@/lib/data";
import { ApologeticsCard } from "@/components/apologetics-card";
import { Shield } from "lucide-react";

interface DefendScreenProps {
  bookmarks: Set<string>;
  onToggleBookmark: (id: string) => void;
}

export function DefendScreen({ bookmarks, onToggleBookmark }: DefendScreenProps) {
  let itemIndex = 0;

  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <h1 className="font-serif text-xl font-bold text-[var(--gold-2)] flex items-center gap-2">
          <span className="text-xl">&#9876;</span>
          Defend
        </h1>
        <p className="text-xs text-muted-foreground mt-1">
          Answers to every Islamic challenge against Christianity
        </p>
      </div>

      {/* Categories */}
      {DEFEND_DATA.map((category) => (
        <div key={category.cat}>
          <div className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground border-b border-border/50 pb-1.5 mb-3">
            {category.cat}
          </div>
          
          {category.items.map((item) => {
            const id = `d${itemIndex++}`;
            return (
              <ApologeticsCard
                key={id}
                item={item}
                type="defend"
                isBookmarked={bookmarks.has(id)}
                onToggleBookmark={() => onToggleBookmark(id)}
              />
            );
          })}
        </div>
      ))}
    </div>
  );
}
